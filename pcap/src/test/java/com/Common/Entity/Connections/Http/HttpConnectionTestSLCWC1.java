...
class HttpConnectionTestSLCWC1 {
    ...
    @BeforeEach
    void setUp() {
        _filePath = Registry.getPCAPFILEPATH() +"http/http-slcw-c-c1.pcap";
        readCommandClient = (ReadPcap) CommandFactory.instantiateReadPcap(_filePath);
        readCommandClient.execute();

        _filePath = Registry.getPCAPFILEPATH() +"http/http-slcw-s-c1.pcap";
    ...
    @Test
    void closingClient() {
        for (int i = 0; i < readCommandClient.getOutput().size() - 6; i++) {
            hc.addPacket(readCommandClient.getOutput().get(i).get(TcpPacket.class));
        }
    ...
    @Test
    void closingServer() {
        for (int i = 0; i < readCommandServer.getOutput().size() - 6; i++) {
            hc.addPacket(readCommandServer.getOutput().get(i).get(TcpPacket.class));
        }
        assertEquals(TelnetConnection.CLOSING, hc.getClosedStatus());
    }
