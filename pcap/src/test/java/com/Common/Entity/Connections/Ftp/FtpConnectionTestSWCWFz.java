...
class FtpConnectionTestSWCWFz {
    ...
    @BeforeEach
    void setUp() {
        _filePath = Registry.PCAPFILEPATH+"ftp/ftp-swcw-c-fz.pcap";
        readCommandClient = (ReadPcap) CommandFactory.instantiateReadPcap(_filePath);
        readCommandClient.execute();

        _filePath = Registry.PCAPFILEPATH+"ftp/ftp-swcw-s-fz.pcap";
    ...
    @Test
    void closingClient() {
        for (int i = 0; i < readCommandClient._output.size() - 5; i++) {
            fc.addPacket(readCommandClient._output.get(i).get(TcpPacket.class));
        }
    ...
    @Test
    void closingServer() {
        for (int i = 0; i < readCommandServer._output.size() - 5; i++) {
            fc.addPacket(readCommandServer._output.get(i).get(TcpPacket.class));
        }
        assertEquals(TelnetConnection.CLOSING, fc.getClosedStatus());
    }
