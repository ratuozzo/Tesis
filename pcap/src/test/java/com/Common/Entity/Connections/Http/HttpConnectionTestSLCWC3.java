...
class HttpConnectionTestSLCWC3 {
    ...
    @BeforeEach
    void setUp() {
        _filePath = Registry.getPCAPFILEPATH() +"http/http-slcw-c-c3.pcap";
        readCommandClient = (ReadPcap) CommandFactory.instantiateReadPcap(_filePath);
        readCommandClient.execute();

        _filePath = Registry.getPCAPFILEPATH() +"http/http-slcw-s-c3.pcap";
    ...
    @Test
    void closingServer() {
        for (int i = 0; i < readCommandServer.getOutput().size() - 2 ; i++) {
            hc.addPacket(readCommandServer.getOutput().get(i).get(TcpPacket.class));
        }
        assertEquals(TelnetConnection.CLOSING, hc.getClosedStatus());
    }
